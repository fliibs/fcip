...
generate
    for(i=0;i<CODE_WIDTH;i++) begin
        for(j=1;j<TOTAL_WIDTH;j++) begin
            if(j&(2**i)) assign dec_check_bits_array[i][j] = enc_data_extend[j];
            else         assign dec_check_bits_array[i][j] = 1'b0;
        end
...
assign sb_err         =  dec_parity_bit && (check_bits_rst <= TOTAL_WIDTH-1);
assign db_err         = (~dec_parity_bit && (|check_bits_rst)) || (check_bits_rst > TOTAL_WIDTH-1);

fcip_bin2onehot #(
    .BIN_WIDTH   (CODE_WIDTH   ),
    .ONEHOT_WIDTH(CODE_WIDTH_OH)
) u_fcip_bin2onehot (
    .bin_in    (check_bits_rst     ),
    .onehot_out(check_bits_rst_ohot)
);

assign dec_data_extend_msk = check_bits_rst_ohot[TOTAL_WIDTH-1:1] ^ enc_data_extend;
